...
export function InputBar({
  ...
  const [workItemDragActive, setWorkItemDragActive] = useState(false)
  ...
  const isZentaoDrag = (types: readonly string[]): boolean => types.some(type => type === ZENTAO_ITEM_MIME)

  const onWorkItemDragOver = (event: React.DragEvent<HTMLTextAreaElement>): void => {
    if (!isZentaoDrag(event.dataTransfer.types)) return
    event.preventDefault()
    event.dataTransfer.dropEffect = !locked && !machineBusy ? 'copy' : 'none'
    setWorkItemDragActive(true)
  }

  const onWorkItemDragLeave = (event: React.DragEvent<HTMLTextAreaElement>): void => {
    if (!isZentaoDrag(event.dataTransfer.types)) return
    setWorkItemDragActive(false)
  }

  const onWorkItemDrop = (event: React.DragEvent<HTMLTextAreaElement>): void => {
    if (!isZentaoDrag(event.dataTransfer.types)) return
    event.preventDefault()
    setWorkItemDragActive(false)
    if (locked || machineBusy) return
    ...
    keyboard.setDraft(next, { start, end, insertedLength: inserted.length })
  ...
  return (
    <div className={clsx(css.root, variant === 'hero' && css.hero)}>
      ...
      <div
        ref={cardRef}
        className={clsx(css.card, workspaceTrigger && css.cardWorkspaceTrigger, workItemDragActive && css.cardWorkItemDrop)}
        data-composer-card
        data-work-item-drop={workItemDragActive || undefined}
        ...
        <div ref={scrollRef} className={css.scroll} data-input-scroll>
          <div className={css.grow}>
            ...
            <textarea
              ...
              onPaste={onPaste}
              onDragEnter={onWorkItemDragOver}
              onDragOver={onWorkItemDragOver}
              onDragLeave={onWorkItemDragLeave}
              onDrop={onWorkItemDrop}

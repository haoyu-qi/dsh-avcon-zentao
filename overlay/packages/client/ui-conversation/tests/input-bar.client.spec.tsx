...
describe('ZenTao work-item drop', () => {
  it('inserts a dragged work item at the textarea selection', () => {
    const { textarea, shell, view } = bench({ draft: '请处理后续事项' })
    textarea.setSelectionRange(3, 3)
    const text = '[禅道 Bug #7｜修复通知刷新](<https://zentao.example.com/index.php?m=bug&f=view&bugID=7>)\n状态：active · 优先级：P1 · 截止 / 更新：2026-08-16\n处理要求：请默认使用禅道 CLI 读取该工作项的最新详情和上下文后再处理。'
    const dataTransfer = {
      types: ['application/x-dsh-zentao-item'],
      getData: (type: string) => type === 'text/plain' ? text : '',
      dropEffect: 'none',
    }
    fireEvent.dragOver(textarea, { dataTransfer })
    expect(dataTransfer.dropEffect).toBe('copy')
    expect(view.container.querySelector('[data-work-item-drop="true"]')).toBeTruthy()
    fireEvent.drop(textarea, { dataTransfer })
    expect(shell.snapshot.draft).toBe(`请处理\n${text}\n后续事项`)
    expect(view.container.querySelector('[data-work-item-drop="true"]')).toBeNull()
  })

  it('ignores a dragged work item while the composer is locked', () => {
    const { textarea, shell } = bench({ draft: '保留原文', inert: true })
    const dataTransfer = {
      types: ['application/x-dsh-zentao-item'],
      getData: () => '[禅道任务 #18] 不应写入',
      dropEffect: 'copy',
    }
    fireEvent.dragOver(textarea, { dataTransfer })
    expect(dataTransfer.dropEffect).toBe('none')
    fireEvent.drop(textarea, { dataTransfer })
    expect(shell.snapshot.draft).toBe('保留原文')
  })
})
